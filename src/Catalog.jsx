...
import { fetchBrigadeCatalog, fetchBrigadeUnits, fetchBrigadeTickets } from "./brigadeStore.js";
...
export default function Catalog({ brigadeId, role, persona, officerUnit, userId, effectiveMemberId, ledTeam, categories, crossNav, clearCrossNav, requestTicketForItem, viewTicketDetail }) {
  ...
  function toggleInterest(id) {
    setCatalog((prev) =>
      prev.map((it) => {
        if (it.id !== id) return it;
        const already = (it.interested || []).some((p) => p.name === currentActor);
        return {
          ...it,
          interested: already
            ? it.interested.filter((p) => p.name !== currentActor)
            : [...(it.interested || []), { name: currentActor }],
        };
      })
    );
  ...
  function decide(id, status, reason) {
    ...
    setCatalog((prev) =>
      prev.map((it) => {
        if (it.id !== id) return it;
        updated = {
          ...it, status, updatedAt: new Date().toLocaleDateString("he-IL"), updatedBy: currentActor,
  ...
  function reopenItem(id, edits) {
    setCatalog((prev) =>
      prev.map((it) =>
        it.id === id
          ? { ...it, ...edits, status: canControlCatalog ? CATALOG_STATUS.ACTIVE : CATALOG_STATUS.PENDING, rejectionReason: null }
          : it
      )
    );
...
.add-item-btn{
  ...
  border-radius:9px; padding:9px 16px; font-family:var(--font-sans); font-weight:700; font-size:13.5px; cursor:pointer;
  transition:filter .15s ease, box-shadow .15s ease; flex:none;
}
...
.prod-card-wrap{ display:flex; flex-direction:column; gap:8px; opacity:0; animation:cardIn .3s ease forwards; }
.prod-card{
...
.prod-card-decide button{
  flex:1; display:inline-flex; align-items:center; justify-content:center; gap:5px; border:none; border-radius:8px;
...
.reject-reason-box{
  ...
  border-radius:9px; padding:9px; width:100%; animation:fadeSlideUp .15s ease;
}
...
.origin-filter-trigger{
  ...
  border-radius:9px; padding:9px 13px; font-size:13px; color:var(--text); cursor:pointer;
  font-family:var(--font-sans); transition:border-color .15s ease; white-space:nowrap;
}
